 ...
 * Only the involved client, advisor, or an admin can access appointment details
 */
...
/**
 * Middleware to check if a user is a advisor and registered by admin
 * Only admin can register advisors - rejects direct advisor registrations
 */
exports.preventAdvisorRegistration = (req, res, next) => {
    // // If user is trying to register as a advisor
    // if (req.body.role === 'advisor') {
    //     // Only allow if the request is from an admin
    //     if (!req.user || req.user.role !== 'admin') {
    //         return res.status(403).json({
    //             message: 'Advisor registration is only available through administrators. Please contact the clinic to register as a advisor.'
    //         });
    //     }
    // }
